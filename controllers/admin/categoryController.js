...
const addCategoryOffer = async(req,res)=>{
    try{
        ...
        if(!category){
            return res.status(404).json({status:false,message:"Ctegory not found"})
        }
        const products = await Product.find({category:category._id})
        const hasProductOffer = products.some((product)=>procduct.hasProductOffer>percentage)
        if(hasProductOffer){
